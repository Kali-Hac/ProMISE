...
tf.app.flags.DEFINE_string('T_prompt', 'MSE',
						   "l1, l2, MSE")  # reconstruction loss type for trajectory prompted reconstruction
tf.app.flags.DEFINE_string('S_prompt', 'MSE',
...
print('----- Original Model (TranSG) hyperparams -----')
...
print('TranSG alpha: ' + FLAGS.seq_lambda)
print('TranSG beta: ' + FLAGS.prompt_lambda)
print('TranSG lambda: ' + FLAGS.GPC_lambda)
...
if FLAGS.mode == 'Train':
	...
	with loaded_graph.as_default():
		...
		with tf.name_scope("TranSG"), tf.variable_scope("TranSG", reuse=tf.AUTO_REUSE):
			...
			D_loss = (1 - float(FLAGS.seq_lambda)) * GPC_ske_loss + float(FLAGS.seq_lambda) * GPC_seq_loss

			SSL_loss = recon_loss

			train_op = optimizer.minimize(
				float(FLAGS.GPC_lambda) * D_loss + (1 - float(FLAGS.GPC_lambda)) * SSL_loss)
		...
		with tf.Session(config=config) as sess:
			...
			node_mask_save = []
			cur_epoch = 0
			global_Tr_mask_num = int(FLAGS.Tr_mask_num)
			global_St_mask_num= int(FLAGS.St_mask_num)
			def train_loader(X_train_J, y_train):
				global mask_rand_save, node_mask_save, mask_rand_save_2
				cur_Tr_mask_num = int(FLAGS.Tr_mask_num)
				cur_St_mask_num = int(FLAGS.St_mask_num)
				...
				while tr_step * batch_size < tr_size:
					...
					# original temporal masking in TranSG
					# rand_choice = np.random.choice(time_step, (time_step - cur_Tr_mask_num),
					# 							   replace=False)
					...
					if FLAGS.St_prompt_type == '1' or FLAGS.St_prompt_type == '2':
						...
						while np.mean(node_mask_rand) == 0:
							prob = np.random.uniform(0, 1, [joint_num, ])
							node_mask_rand = prob >= float(FLAGS.prob_s)
						# original spatial masking in TranSG
						# node_mask_rand = np.ones((joint_num), dtype=bool)
						# rand_list = np.random.choice(joint_num, size=cur_St_mask_num, replace=False)
						# node_mask_rand[rand_list] = False
			...
			for epoch in range(cluster_epochs):
				...
				if epoch > 0:
					if dataset == 'CASIA_B':
						print(
							'[TranSG - Probe Evaluation] %s - %s | mAP: %.4f (%.4f) | Top-1: %.4f (%.4f) | Top-5: %.4f (%.4f) | Top-10: %.4f (%.4f) | ' % (
				...
				while tr_step * batch_size < tr_size:
					...
					if FLAGS.rand_flip == '1':
						...
						if FLAGS.save_flag == '0':
							_,  SSL_loss_,GPC_loss_, Seq_features = sess.run(
								[train_op, SSL_loss, D_loss, seq_ftr],
								feed_dict={
									J_in: X_input_J,
									seq_mask: mask_rand,
						...
						# # #
							...
							_, spatial_loss_, temporal_loss_, SSL_loss_, GPC_loss_, Seq_features = sess.run(
								[train_op, spatial_loss, temporal_loss, SSL_loss, D_loss, seq_ftr],
								feed_dict={
									J_in: X_input_J,
									seq_mask: mask_rand,
									seq_mask_2: mask_rand_2,
					...
					if FLAGS.save_flag == '1':
						batch_SSL_loss.append(SSL_loss_)
						batch_spatial_loss.append(spatial_loss_)
						batch_temporal_loss.append(temporal_loss_)
						batch_GPC_loss.append(GPC_loss_)
					...
					tr_step += 1
				if FLAGS.save_flag == '1':
					SSL_losses.append(np.mean(batch_SSL_loss))
					spatial_losses.append(np.mean(batch_spatial_loss))
					temporal_losses.append(np.mean(batch_temporal_loss))
					GPC_losses.append(np.mean(batch_GPC_loss))
			...
			if FLAGS.save_flag == '1':
				...
				np.save(pre_dir + dataset + '/' + probe + change + '/' + 'SSL_loss.npy', SSL_losses)
				np.save(pre_dir + dataset + '/' + probe + change + '/' + 'spatial_loss.npy', spatial_losses)
				np.save(pre_dir + dataset + '/' + probe + change + '/' + 'temporal_loss.npy', temporal_losses)
				np.save(pre_dir + dataset + '/' + probe + change + '/' + 'GPC_loss.npy', GPC_losses)
	...
	with tf.Session(graph=loaded_graph, config=config) as sess:
		...
		seq_ftr = loaded_graph.get_tensor_by_name("TranSG/TranSG/Reshape_41:0")
